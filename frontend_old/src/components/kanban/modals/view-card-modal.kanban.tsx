...
export const ViewCardModal = (props: any) => {
  ...
  useEffect(() => {
    const getCardUsers = async () => {
      props.card.users.forEach(async (userId) => {
        ...
        if (result.status === 200) {
          Array.from(result.data).map((user) =>
    ...
    // getCardUsers()
    // console.log('users', props.card.users)
    console.log('load card modal')
    console.log({ issueId })
  }, [])
  ...
  // const onSelectedUsersChange = async (user) => {
  //   const response = await fetch(`${getApiUrl()}/kanban/cards/${props.card._id}`, {
  //     method: 'PUT',
  //     headers: {
  //       'Content-Type': 'application/json'
  ...
  //   if (result.status === 200) {
  //     setCard({ ...card, users: result.data.users })
  //     setSelectedUsers((prevState) => [...prevState, user])
  //     console.log('selectedUsersFromCard', selectedUsers)
  //   }
  // }
                  ...
                  {!selectedUsers.length ? (
                    ...
                    <>
                      {selectedUsers.map((user, index) => (
                        <div
                          style={{ width: '5%' }}
                          className='d-flex align-items-center justify-content-center bg-info rounded-circle'
                          key={index}
                        >
